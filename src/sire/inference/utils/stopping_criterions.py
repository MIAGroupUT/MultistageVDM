...
class NotAVesselStoppingCriterion(StoppingCriterionBase):
    '''
    Stopping criterion based on the circularity of the tracked structure.
    '''
    def __init__(self, contour_name: str = "lumen", min_circularity: float = 0.4):
    ...
    def __call__(self, vessel_contour: VesselContour, **kwargs):
        ...
        return vessel_contour._estimate_circularity() < self.min_circularity

    def __str__(self):
        return f"{self.__class__.__name__}(circularity={self.min_circularity})"
...
class RoiStoppingCriterion(StoppingCriterionBase):
    def __init__(self, 
        ...
        self.track_axis = track_axis # If we need to know from where the tracker is exiting.

    def _in_axes(self, coords: torch.tensor):
        axes = len(self.roi.shape)
        check_per_axes = torch.stack([0 <= coords[axis] <= self.roi.shape[axis] for axis in range(axes)])

        return torch.all(check_per_axes), check_per_axes
    ...
    def __call__(self, data: Dict[str, Any], point: torch.tensor, **kwargs):
        ...
        out_check, axes_check = self._in_axes(point_physical)

        if not out_check: # If it is out in z, at least for this case, that is useful info.
            print(axes_check)
            return True
        else:
            try:
